...
class Player {
    ...
    public static void main(String args[]) {
        ...
        while (true) {
            ...
            if (rep != -1) { //if this node is linked to a Gateway
                ...
                System.err.println(isolateGateWay(links, gateways));
            }
    ...
    public static int isLinkedToGateWay(int[][] links, List<Integer> gateways, int node) {
        System.err.println("Kiem tra node " + node + " co phai gw?");
        int ret = -1;
        int go = 1;
        ...
        for (int k = 0; k < gateways.size() && go == 1; k++) {
            ...
            while (go == 1 && i < N) {
                ...
                System.err.println("Link value: " + links[i][node]);
                System.err.println("Gw value: " + gateways.get(k));
                if (links[i][node] == 1 && i == gateways.get(k)) {
                    go = 0;
                    ret = i;
                } else {
                    go = 1;
    ...
    public static String isolateGateWay(int[][] links, List<Integer> gateways) {
        ...
        if (node != -1) {
            ...
            links[node][gateways.get(0)] = 0;
            links[gateways.get(0)][node] = 0;
        }
